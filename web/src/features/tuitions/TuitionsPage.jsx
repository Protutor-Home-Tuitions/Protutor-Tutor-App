/**
 * TuitionsPage — main tuitions dashboard.
 */
...
import { ALL_CITIES } from 'protutor-shared'
...
export default function TuitionsPage() {
  ...
  const { isManager, cityAllowed }  = useAuthStore()
  ...
  const filtered = useMemo(() => tuitions.filter((t) => {
    ...
    return true
  }), [tuitions, search, filter, cityFilter])
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        ...
        {isManager() && <Button onClick={() => {}}>+ Add Tuition</Button>}
      </div>

      <div className="flex gap-3 mb-5 flex-wrap items-center">
        <input
          type="text"
          placeholder="Search student, parent, phone, ID..."
          value={search}
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <TuitionRow key={t.id} tuition={t} prevMonth={prevMonth}
                  onView={setDetailId} onEdit={() => {}} onToggle={() => {}} />
              ))}
            </tbody>
          </table>
        </div>
