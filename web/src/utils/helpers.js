...
export function buildTutorWAMessage({ tuition, tutor, senderName }) {
  ...
  const [, m] = (tuition.start || '').split('-')
  const startMonth = m ? MN_FULL[parseInt(m) - 1] : ''
  ...
  return [
    ...
    `As discussed, the classes have started from ${startDate} ${startMonth} for ${tuition.studentName}.`,
    '',
...
export function buildParentWAMessage({ tuition, tutor, senderName }) {
  ...
  return [
    ...
    `${tutor?.name || 'Your tutor'} has been assigned to start classes for your child from ${startDate}, ${startMonth}.`,
    '',
    '*Class Details*',
