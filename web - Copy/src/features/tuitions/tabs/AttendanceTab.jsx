/**
 * AttendanceTab.jsx — full migration of renderAttendanceTab()
 */
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {canWrite() && (
          <button onClick={() => { setEditingAtt(null); setAddModalOpen(true) }}
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => downloadPDF(t, tutor, allAtt, monthKey)}
          ...
          {monthKey && monthKey <= thisMonth && (
            <BillingBtn activeBill={activeBill} attSubmitted={attSubmitted} newerBill={newerBill}
              canCreate={isManager() || isCoordinator()} />
          )}
        </div>
      </div>
...
function BillingBtn({ activeBill, attSubmitted, newerBill, canCreate }) {
  if (activeBill) {
    return activeBill.zeroBill
      ...
      : <div className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-100 text-green-700 text-xs font-semibold rounded-lg"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><path d="M20 6L9 17l-5-5"/></svg>Billing Created</div>
  }
  if (!canCreate) return null
  if (newerBill) return <span className="text-xs text-slate-400 italic">Locked — newer bill exists</span>
  if (!attSubmitted) return <span className="text-xs text-slate-400 italic">Awaiting att submission</span>
  return (
...
function AttTable({ rows, tuition, onEdit, onDelete, canWrite, isManager }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
      <div className="grid bg-slate-50 border-b border-slate-200" style={{ gridTemplateColumns: cols }}>
        {headers.map((h) => <div key={h} className="px-3 py-2.5 text-xs font-bold text-slate-400 uppercase tracking-wide">{h}</div>)}
      </div>
      {rows.map((a, i) => {
        const dow = DOW[new Date(a.date + 'T00:00:00').getDay()]
        const mt  = a.markedAt ? new Date(a.markedAt) : null
        const markedDate = mt ? `${String(mt.getDate()).padStart(2,'0')}/${String(mt.getMonth()+1).padStart(2,'0')}/${String(mt.getFullYear()).slice(2)}` : null
        const markedTime = mt ? mt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }) : null
        const durMismatch = !a.isDemo && tuition?.duration != null && parseFloat(a.dur) !== parseFloat(tuition.duration)
        ...
        return (
          <div key={a.id} className="grid border-b border-slate-100 items-center"
            ...
            <div className="px-3 py-2.5 flex items-center gap-1">
              ...
              {durMismatch && (
                <span title={`Scheduled: ${tuition.duration}hr · Marked: ${a.dur}hr`}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#D97706" strokeWidth="2.2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                </span>
              )}
            </div>
            ...
            <div className="px-3 py-2.5 text-xs text-slate-600 leading-relaxed">
              {markedDate ? <>{markedDate}<br /><span className="text-slate-400">{markedTime}</span></> : '—'}
            </div>
...
function downloadPDF(t, tutor, allAtt, monthKey) {
  ...
  const rows = [...monthAtt].sort((a,b) => b.date.localeCompare(a.date)).map((a) => {
    ...
    return `<tr style="${a.isDemo?'background:#fefce8;':''}"><td>${fd(a.date)}<br><small style="color:#888">${dow}</small>${a.isDemo?'<br><span style="font-size:9px;background:#fef9c3;color:#ca8a04;padding:1px 4px;border-radius:3px;font-weight:700">DEMO</span>':''}</td><td>${a.time}</td><td>${a.dur}hr</td><td>${a.subj||'—'}</td><td>${a.topic||'—'}</td><td style="font-size:11px">${ms}</td></tr>`
  }).join('')
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Attendance — ${t.parentName||t.studentName} — ${monthLabel}</title>
  <style>body{font-family:Arial,sans-serif;font-size:13px;color:#0F172A;margin:30px}h2{font-size:18px;margin-bottom:4px}.meta{font-size:12px;color:#475569;margin-bottom:20px;line-height:1.8}.stats{display:flex;gap:24px;margin-bottom:20px}.stat{background:#f7f9ff;border:1px solid #dbeafe;border-radius:8px;padding:10px 18px;text-align:center}.stat-val{font-size:22px;font-weight:700;color:#1A56DB}.stat-lbl{font-size:11px;color:#64748B}table{width:100%;border-collapse:collapse;margin-top:8px}th{background:#1A56DB;color:white;padding:8px 10px;font-size:11px;text-align:left;letter-spacing:0.4px}td{padding:8px 10px;border-bottom:1px solid #E2E8F0;font-size:12px;vertical-align:top}tr:nth-child(even){background:#F7F9FF}.footer{margin-top:24px;font-size:11px;color:#94A3B8;border-top:1px solid #E2E8F0;padding-top:10px}</style></head>
  <body><h2>${t.parentName||t.studentName} — ${t.studentName}</h2>
  <div class="meta">Enquiry ID: ${t.enqId} &nbsp;|&nbsp; Tutor: ${tutor?.name||'—'} &nbsp;|&nbsp; Month: ${monthLabel}<br>Subjects: ${t.subjects?.join(', ')} &nbsp;|&nbsp; Days: ${t.days?.join(', ')} &nbsp;|&nbsp; Duration: ${t.duration}hr/day</div>
  <div class="stats"><div class="stat"><div class="stat-val">${nonDemo.length}</div><div class="stat-lbl">Classes</div></div><div class="stat"><div class="stat-val">${totalHours}hr</div><div class="stat-lbl">Total Hours</div></div><div class="stat"><div class="stat-val">${monthFee!==null?'₹'+monthFee.toLocaleString('en-IN'):'—'}</div><div class="stat-lbl">Fee (P)</div></div></div>
  ...
  <div class="footer">Generated by ProTutor Admin &nbsp;·&nbsp; ${new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</div></body></html>`
